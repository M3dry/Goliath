...
pub const InputState = struct {
    ...
    pub fn init(self: *InputState, window: *zglfw.Window) void {
        self.* = .{};
    ...
    pub fn deinit(self: *InputState, window: *zglfw.Window) void {
    ...
    pub fn update(self: *InputState, capture: CaptureFilter) void {
        self.key_just_pressed = .initEmpty();
        self.key_just_released = .initEmpty();
        self.key_repeated = .initEmpty();
        self.mouse_just_pressed = .initEmpty();
    ...
    pub fn isDown(self: *const InputState, key: Key) bool {
        if (key == .unknown) return false;
        return self.key_down.isSet(@intCast(@intFromEnum(key)));
    }

    pub fn justPressed(self: *const InputState, key: Key) bool {
        if (key == .unknown) return false;
        return self.key_just_pressed.isSet(@intCast(@intFromEnum(key)));
    }

    pub fn justReleased(self: *const InputState, key: Key) bool {
        if (key == .unknown) return false;
        return self.key_just_released.isSet(@intCast(@intFromEnum(key)));
    }

    pub fn isRepeated(self: *const InputState, key: Key) bool {
        if (key == .unknown) return false;
        return self.key_repeated.isSet(@intCast(@intFromEnum(key)));
    }

    pub fn anyJustPressed(self: *const InputState) ?Key {
        return self.last_just_pressed;
    }

    pub fn anyKeyDown(self: *const InputState) bool {
        return self.key_down.findFirstSet() != null;
    }

    pub fn mousePos(self: *const InputState) struct { x: f64, y: f64 } {
        return .{ .x = self.mouse_x, .y = self.mouse_y };
    }

    pub fn mouseDelta(self: *const InputState) struct { x: f64, y: f64 } {
    ...
    pub fn mouseJustPressed(self: *const InputState, button: MouseButton) bool {
        return self.mouse_just_pressed.isSet(@intCast(@intFromEnum(button)));
    }

    pub fn mouseJustReleased(self: *const InputState, button: MouseButton) bool {
        return self.mouse_just_released.isSet(@intCast(@intFromEnum(button)));
    }

    pub fn justPressedWith(self: *const InputState, key: Key) ?Mods {
    ...
    pub fn mouseJustPressedWith(self: *const InputState, button: MouseButton) ?Mods {
    ...
    pub fn charEvents(self: *const InputState) []const u21 {
        return self.char_events[0..self.char_count];
    }

    pub fn clearCharEvents(self: *InputState) void {
        self.char_count = 0;
    }
};
...
fn keyCallback(window: *zglfw.Window, key: Key, scancode: c_int, action: Action, mods: Mods) callconv(.c) void {
    ...
    const state = window.getUserPointer(InputState) orelse return;
