...
sampled_textures: std.MultiArrayList(struct {
    ...
    texture_gid: Gid = .{ .gen = std.math.maxInt(u32), .slot = std.math.maxInt(u32) },
    sampler: base.Sampler = .null_handle,
    default_image: DefaultKind = .white,
    in_texture_pool: bool = false,
}) = .empty,
...
pub fn deinit(self: *TextureRegistry, ctx: base.Ctx.Query(&.{ .destroy_queue, .transport })) void {
    ...
    for (sampled_textures_slice.items(.sampler)) |*sampler| {
...
pub fn deinitNow(self: *TextureRegistry, ctx: base.Ctx.Query(&.{ .device, .vma_allocator })) void {
    const textures_slice = self.textures.slice();
    for (textures_slice.items(.image)) |*tex| {
...
pub fn acquireSampledTexture(self: *TextureRegistry, ctx: base.Ctx.Query(&.{ .device, .vma_allocator, .transport }), io: std.Io, loader: *Loader, resolved: resolver.Resolved, cold: *const Loader.ColdAsset, id: u32) !void {
    ...
    slice.items(.texture_gid)[id] = blob.texture_gid;
    ...
    if (try ctx.view.transport.isReady(ticket)) {
        ...
        slice.items(.in_texture_pool)[id] = true;
    } else {
...
pub fn releaseSampledTexture(self: *TextureRegistry, ctx: base.Ctx.Query(&.{ .device, .destroy_queue }), id: u32) !void {
    ...
    sampler.deinit(.from(ctx));
    slice.items(.sampler)[id] = .null_handle;
    slice.items(.in_texture_pool)[id] = false;
...
pub fn ingestTexture(io: std.Io, location: types.IngestLocation, tex: IngestTexture) types.IngestError!Entry {
    ...
    const blob_size = switch (tex.data) {
        .decoded_blob => |d| blk: {
            try writer.writeStruct(TextureBlobHeader{ .format = d.format, .width = d.width, .height = d.height, .default_image = tex.default_image }, .little);
...
fn resizeTexturePool(self: *TextureRegistry, ctx: base.Ctx.Query(&.{ .device, .destroy_queue }), new_cap: u32) !void {
    ...
    for (0.., slice.items(.in_texture_pool), slice.items(.sampler)) |id, in_texture_pool, sampler| {
        if (!in_texture_pool) {
