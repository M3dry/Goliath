...
pub fn raster(self: *Self, current_frame: u32, rg: *base.RenderGraph, vis_ref: base.RenderGraph.ImageRef, vp: base.zmath.Mat, depth_attachment: base.RenderGraph.DepthAttachment, renderables_buf_ref: base.RenderGraph.BufferRef, renderables_buf_address: u64, draw_cmds_buf_ref: base.RenderGraph.BufferRef, draw_cmds_buf_address: u64, max_draw_count: u32, pc_buf: *[pc_size]u8) !void {
    base.push_constant.write(VisPC, pc_buf, .{
        .vp = vp,
        .renderables_address = renderables_buf_address,
        .draw_buffer_address = draw_cmds_buf_address,
    }, base.layout.scalar);
    ...
    const pass = try rg.addGraphicsPass(.{
        ...
        .color_attachments = &.{.{
            .image = vis_ref,
            .view = self.views[current_frame].handle,
            .load_op = .clear,
            .store_op = .store,
            .clear_color = .{
                .uint_32 = .{ 0, 0, 0, 0 }
    ...
    try pass.readBuffer(renderables_buf_ref, .{
        .stage = .{ .vertex_shader_bit = true },
        .access = .{ .shader_read_bit = true },
    });
    try pass.readBuffer(draw_cmds_buf_ref, .{
        .stage = .{ .vertex_shader_bit = true },
        .access = .{ .shader_read_bit = true },
    });

    pass.drawIndirectCount(.{
        .push_constant = pc_buf,
        .buffer = draw_cmds_buf_ref,
        .offset = 0,
        .count_buffer = renderables_buf_ref,
        .count_offset = 0,
